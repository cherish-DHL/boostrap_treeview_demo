	    ...
	    this.forEachIdentifier(identifiers, options, $.proxy(function (node, options) {
			...
			if(parentNode && parentNode.nodes != null ){
				if(parentNode.nodes.length > 1){
					BootstrapDialog.confirm('此节点下可能存在其他节点，确定要删除么？', function(result){
			            if(result) {
					...
					BootstrapDialog.confirm('确定要删除么？', function(result){
			            if(result) {
							parentNode.nodes.splice(0, 1);
			            }
			        });
			        this.setInitialStates({ nodes: this.tree }, 0);
	                this.render();
	            ...
	            BootstrapDialog.confirm('此节点是根节点，确定要删除么？', function(result){
		            if(result) {
		               node.nodes = null;
			           node.text = '';
                        ...
                        setTimeout(function(){  //set auto close  time
						    dlg.close();
						},1000);
			        }else{
                        var dlg = BootstrapDialog.show({
